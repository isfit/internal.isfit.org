class Editorial::ArticlesController < ApplicationController
  ...
  def attending
    ...
    if(ArticlesAttending.where(:user_id => @user_id, :article_id => @article_id).blank?)
      ...
      if @article_attending.save
        redirect_to editorial_article_path(@article_id), notice: 'You was successfully added to this event.'
      else
  ...
  def notattending
    ...
    if(!ArticlesAttending.where(:user_id => @user_id, :article_id => @article_id).blank?)
      ...
      redirect_to editorial_article_path(@article_id), notice: 'You was successfully removed from this event.'
    else
  ...
  def create
    ...
    respond_to do |format|
      if @article.save
        ...
        format.html { redirect_to [:editorial, @article], notice: 'Article was successfully created.' }
  ...
  def update
    ...
    if @article.update_attributes(params[:article])
      redirect_to [:editorial,@article], notice: 'Article was successfully updated.'
    else
      render :edit
    end
  end
