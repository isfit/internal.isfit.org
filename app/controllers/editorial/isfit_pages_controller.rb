class Editorial::IsfitPagesController < ApplicationController
  # GET /isfit_pages
  ...
  def create
    ...
    respond_to do |format|
      if @isfit_page.save
        format.html { redirect_to [:editorial, @isfit_page], notice: 'Isfit page was successfully created.' }
  ...
  def update
    ...
    respond_to do |format|
      if @isfit_page.update_attributes(params[:isfit_page])
        format.html { redirect_to [:editorial, @isfit_page], notice: 'Isfit page was successfully updated.' }
        format.json { head :ok }
      else
